                        ...
                        <div class="table-responsive">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($item->reviews->where('user_id',$item->tutor_id)->first()->rating ?? false)
                                        @for ($i = 0; $i < 5; $i++)
                                        @if (floor($item->reviews->where('user_id',$item->tutor_id)->first()->rating) - $i >= 1)
                                        {{--Full Star--}}
                                        <i style="color: #FFC125;" class="fa fa-star"></i>
                                        @elseif ($item->reviews->where('user_id',$item->tutor_id)->first()->rating - $i > 0)
                                        {{--Half Star--}}
                                    ...
                                    <td>
                                        ...
                                        @else
                                            @if($item->status=='2')
                                                <a href="{{route('student.review.add', $item->id)}}" class="btn btn-primary rounded-circle btn-round">
                                                    <i class="fa fa-comments-o" aria-hidden="true"></i>
                                                </a>
                                            @endif
                                        @endif
                                        @if($item->status=='1')
                                            <a href="{{ $item->join_url }}" target="_blank" class="btn btn-primary shadow rounded-pill btn-sm">
                                                Join Meeting
                                            </a>
                                        @endif
                                    </td>
